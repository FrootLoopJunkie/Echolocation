...
io.on('connection', async(socket) => {
    ...
    socket.on('roomRequest', async(arg) => {
        ...
        try{
            const client = await pool.connect();
            const roomPosts = await pool.query('SELECT * FROM (SELECT * FROM post_hashtags ORDER BY post_id DESC LIMIT 1) AS hashtagposts INNER JOIN posts_public ON post_hashtags.post_id = posts_public.post_id');
